...
#define TEST(desc)                                                                                 \
    for (int __test_once =                                                                         \
             (g_test.test_name = desc, g_test.file = __FILE__, g_test.line = __LINE__,             \
             g_test.passed = 1, g_test.skipped = 0, g_test.is_running = 1, g_total_tests++,        \
             (g_verbose ? (void)printf("  running: %s\n", desc) : (void)0), 1);                    \
         __test_once; __test_once = 0)                                                             \
...
#define ASSERT_EQ_INT(expected, actual)                                                            \
    do {                                                                                           \
        ...
        if (_exp != _act) {                                                                        \
            g_test.passed = 0;                                                                     \
            printf("    FAILED at %s:%d  expected %" PRId64 ", got %" PRId64 "\n", __FILE__, __LINE__, \
                   (int64_t)_exp, (int64_t)_act);                                                  \
            g_failed_tests++;                                                                      \
        }                                                                                          \
    } while (0)
...
#define ASSERT_EQ_UINT(expected, actual)                                                           \
    do {                                                                                           \
        ...
        if (_exp != _act) {                                                                        \
            g_test.passed = 0;                                                                     \
            printf("    FAILED at %s:%d  expected 0x%016" PRIx64 ", got 0x%016" PRIx64 "\n", __FILE__, __LINE__, \
                   (uint64_t)_exp, (uint64_t)_act);                                                \
            g_failed_tests++;                                                                      \
        }                                                                                          \
    } while (0)
...
#define ASSERT_NE_INT(expected, actual)                                                            \
    do {                                                                                           \
        ...
        if (_exp == _act) {                                                                        \
            g_test.passed = 0;                                                                     \
            printf("    FAILED at %s:%d  values equal (%" PRId64 ")\n", __FILE__, __LINE__,        \
                   (int64_t)_act);                                                                 \
            g_failed_tests++;                                                                      \
        }                                                                                          \
    } while (0)
...
#define ASSERT_MEM_EQ(ptr1, ptr2, len)                                                             \
    do {                                                                                           \
